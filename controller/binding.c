                             ...
                             struct local_datapath *ld)
{
    ...
    struct local_datapath *peer_ld =
        get_local_datapath(b_ctx_out->local_datapaths, peer->datapath->tunnel_key);
    if (!peer_ld) {
                                     ...
                                     struct binding_ctx_out *b_ctx_out)
{
    ...
    OVSREC_INTERFACE_TABLE_FOR_EACH_TRACKED (iface_rec, b_ctx_in->iface_table) {
        ...
        if (cleared_iface_id) {
            ...
            if (lbinding && lbinding->pb &&
                ...
                if (ld) {
                    remove_pb_from_local_datapath(lbinding->pb, b_ctx_in->chassis_rec,
                                                  b_ctx_out, ld);
                }
    ...
    if (qos_map_ptr && set_noop_qos(b_ctx_in->ovs_idl_txn, b_ctx_in->port_table,
                                    b_ctx_in->qos_table, b_ctx_out->egress_ifaces)) {
        const char *entry;
        SSET_FOR_EACH (entry, b_ctx_out->egress_ifaces) {
            setup_qos(entry, &qos_map);
                                    ...
                                    struct binding_ctx_out *b_ctx_out)
{
    ...
    SBREC_PORT_BINDING_TABLE_FOR_EACH_TRACKED (pb, b_ctx_in->port_binding_table) {
        ...
        if (is_deleted) {
            ...
            if (ld) {
                remove_pb_from_local_datapath(pb, b_ctx_in->chassis_rec, b_ctx_out, ld);
            }
        } else {
            if (consider_for_vif) {
                if (lbinding) {
