...
release_local_binding(struct local_binding *lbinding,
                      ...
                      struct hmap *tracked_dp_bindings)
{
    if (!release_local_binding_children(lbinding, sb_readonly,
                                        tracked_dp_bindings)) {
        return false;
    }

    if (!release_lport(lbinding->pb, sb_readonly)) {
        return false;
    }
                         ...
                         struct hmap *qos_map)
{
    ...
    if (!parent_lbinding) {
        ...
        parent_lbinding = local_binding_create(pb->parent_port, NULL, parent,
                                               BT_VIF);
    ...
    SBREC_PORT_BINDING_TABLE_FOR_EACH (pb,
        ...
        case LP_VIF:
            if (is_lport_container(pb)) {
                ...
                consider_vif_lport(pb, b_ctx_in, b_ctx_out, qos_map_ptr);
            }
            break;
                 ...
                 struct hmap *qos_map)
{
    ...
    if (lbinding->pb) {
        if (!consider_vif_lport(lbinding->pb, b_ctx_in, b_ctx_out, qos_map)) {
            return false;
        }
    }
                    ...
                    struct binding_ctx_out *b_ctx_out, bool *changed)
{
    ...
    if (lbinding && lbinding->pb &&
        ...
        if (!release_local_binding(lbinding, !b_ctx_in->ovnsb_idl_txn,
                                   b_ctx_out->tracked_dp_bindings)) {
            return false;
        }
                         ...
                         bool *changed)
{
    ...
    if (lport_type == LP_VIRTUAL) {
        ...
        handled = consider_vif_lport(pb, b_ctx_in, b_ctx_out, qos_map);
    }
