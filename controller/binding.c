...
static struct binding_lport *binding_lport_update(
    struct shash *binding_lports, struct binding_lport *,
    const struct sbrec_port_binding *pb, enum en_lport_type);
...
delete_done:
    ...
    SBREC_PORT_BINDING_TABLE_FOR_EACH_TRACKED (pb,
        ...
        if (b_lport) {
            ...
            /* binding_lport_update() may destroy 'b_lport'. So don't access it
             * after the return. */
            binding_lport_update(&b_ctx_out->lbinding_data->binding_lports,
                                 b_lport, pb, lport_type);

            if (lbinding) {
                handled = local_binding_handle_stale_binding_lports(
                    lbinding, b_ctx_in, b_ctx_out, qos_map_ptr);
                if (!handled) {
...
static struct binding_lport *
binding_lport_update(struct shash *binding_lports,
                     struct binding_lport *b_lport,
                     const struct sbrec_port_binding *pb,
                     enum en_lport_type lport_type)
{
    /* If lport_type is not LP_VIF, LP_VIRTUAL or LP_CONTAINER, then
     * delete the b_lport. */
    if (lport_type != LP_VIF && lport_type != LP_CONTAINER &&
            lport_type != LP_VIRTUAL) {
        binding_lport_delete(binding_lports, b_lport);
        return NULL;
    }
