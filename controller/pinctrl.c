...
process_packet_in(struct rconn *swconn, const struct ofp_header *msg)
{
    ...
    case ACTION_OPCODE_HANDLE_SVC_CHECK:
        ovs_mutex_lock(&pinctrl_mutex);
        pinctrl_handle_svc_check(swconn, &headers, &packet, &pin.flow_metadata);
        ovs_mutex_unlock(&pinctrl_mutex);
        break;
                     ...
                     uint16_t ip_payload_len)
{
    ...
    packet_set_ipv4(packet, ipv4_src, ipv4_dst, 0, 63);
                      ...
                      long long int current_time)
{
    ...
    pinctrl_compose_ipv4(&packet, ip_ms->cfg.query_eth_src,
                         ...
                         IPPROTO_IGMP, sizeof(struct igmpv3_query_header));
    ...
    LIST_FOR_EACH_SAFE (svc_mon, next, list_node, &svc_monitors) {
        if (svc_mon->delete) {
            ...
            if (svc_mon->status != SVC_MON_ST_UNKNOWN) {
                ...
                } else if (svc_mon->status == SVC_MON_ST_OFFLINE) {
                    sbrec_service_monitor_set_status(svc_mon->sb_svc_mon, "offline");
                } else {
                    sbrec_service_monitor_set_status(svc_mon->sb_svc_mon, "error");
                }
            }
        }
    }
                                    ...
                                    ovs_be16 tcp_src)
{
    ...
    struct eth_addr eth_src;
    eth_addr_from_string("9a:2a:32:f5:89:07", &eth_src);
    char *s_ip = "10.0.0.9";
    ovs_be32 ip4_src;
    ip_parse(s_ip, &ip4_src);

    dp_packet_use_stub(&packet, packet_stub, sizeof packet_stub);
    pinctrl_compose_ipv4(&packet, eth_src, svc_mon->ea,
                         ip4_src, in6_addr_get_mapped_ipv4(&svc_mon->ip),
                         IPPROTO_TCP, TCP_HEADER_LEN);
                              ...
                              struct svc_monitor *svc_mon)
{
    ...
    svc_mon->wait_time = time_msec() + 3000;
    ...
    LIST_FOR_EACH (svc_mon, list_node, &svc_monitors) {
        ...
        case SVC_MON_S_WAITING:
            if (current_time > svc_mon->wait_time) {
                ...
                svc_mon->next_send_time = current_time + 5000;
                             ...
                             struct svc_monitor *svc_mon)
{
    ...
    if ((TCP_FLAGS(th->tcp_ctl) & (TCP_SYN | TCP_ACK))
        ...
        svc_monitor_send_tcp_health_check__(swconn, svc_mon, TCP_RST | TCP_ACK,
                                            htonl(tcp_ack + 1), htonl(tcp_seq + 1),
                                            th->tcp_dst);
        /* Calculate next_send_time. */
        svc_mon->next_send_time = time_msec() + 5000;
        return true;
    }
    ...
    if (in_ip->ip_proto == IPPROTO_TCP) {
        ...
        struct svc_monitor *svc_mon =
            pinctrl_find_svc_monitor(dp_key, port_key, &ip_addr,
                                     ntohs(ip_flow->tp_src), hash);
        if (!svc_mon) {
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
            VLOG_WARN_RL(&rl, "handle service check: Service monitor not found");
            return;
        }
        pinctrl_handle_tcp_svc_check(swconn, pkt_in, svc_mon);
    } else {
        VLOG_INFO("NUMS : %s : %d : ICMP packet dude", __FUNCTION__, __LINE__);
        /* It's ICMP packet. */
        ...
        VLOG_INFO("NUMS : %s : %d : ICMP packet dude 22222", __FUNCTION__, __LINE__);
        const char *end = (char *)dp_packet_l4(pkt_in) + dp_packet_l4_size(pkt_in);

        const struct ip_header *orig_ip_hr = dp_packet_get_icmp_payload(pkt_in);
        if (!orig_ip_hr) {
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
            VLOG_WARN_RL(&rl, "Original IP datagram not present in ICMP packet");
            return;
        }

        if (ntohs(orig_ip_hr->ip_tot_len) != (IP_HEADER_LEN + UDP_HEADER_LEN + 8)) {
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
            VLOG_WARN_RL(&rl, "Invalid original IP datagram length present in ICMP packet");
            return;
        }

        struct udp_header *orig_uh = (struct udp_header *) (orig_ip_hr + 1);
        VLOG_INFO("NUMS : %s : %d : UDP - IP = [%d]", __FUNCTION__, __LINE__, (int)((char *)orig_uh - (char *)orig_ip_hr));
        if ((char *)orig_uh >= end) {
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
            VLOG_WARN_RL(&rl, "Invalid UDP header in the original IP datagram");
            return;
        }

        VLOG_INFO("Originak udp hdr src port = [%d] : dst port = [%d]", ntohs(orig_uh->udp_src), ntohs(orig_uh->udp_dst));
        uint32_t hash =
            hash_bytes(&ip_addr, sizeof ip_addr,
        ...
        struct svc_monitor *svc_mon =
            pinctrl_find_svc_monitor(dp_key, port_key, &ip_addr,
                                     ntohs(orig_uh->udp_dst), hash);
        if (!svc_mon) {
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
            VLOG_WARN_RL(&rl, "handle service check: Service monitor not found for ICMP packet");
            return;
        }

        if (orig_uh->udp_src != svc_mon->tp_src) {
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
            VLOG_WARN_RL(&rl, "handle service check: UDP src port doesn't match in the Original IP datagram of ICMP packet");
            return;
        }
