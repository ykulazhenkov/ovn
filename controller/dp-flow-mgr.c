...
struct ovn_flow_action {
    struct hmap_node uuid_action_node;
...
struct ovn_flow_table {
    ...
    /* Hash map flow table using uuid as hash key.*/
    struct hmap uuid_action_flow_table;
};
...
static bool ovn_flow_update_actions(struct ovn_flow_table *,
                                    struct ovn_flow *,
                                    struct ovn_flow_action *);
static bool ovn_flow_table_is_empty(struct ovn_flow_table *);
                       ...
                       const struct uuid *flow_uuid)
{
    ...
    f->match_flow_table = &active_ftable->match_flow_table; 
    ...
    if (!ovn_flow_table_is_empty(old_ftable)) {
        ...
        if (existing_f) {
            ...
            if (existing_f_a) {
                ...
                if (!ovn_flow_has_active_actions(existing_f)) {
                    hmap_remove(&old_ftable->match_flow_table,
                ...
                const struct uuid *flow_uuid)
{
    ...
    HMAP_FOR_EACH_SAFE (a, next, uuid_action_node,
                        &flow_table->uuid_action_flow_table) {
        if (uuid_equals(&a->flow_uuid, flow_uuid)) {
            struct ovn_flow *f = a->flow;
            ovn_flow_unref_action(f, a);
            hmapx_add(modified_flows, (void *) f);
        }
    }
}
...
ovn_flow_table_clear(struct ovn_flow_table *flow_table)
{
    ...
    HMAP_FOR_EACH_POP (a, uuid_action_node,
                       &flow_table->uuid_action_flow_table) {
...
ovn_flow_clear_actions_from_list(struct ovs_list *act_list)
{
    ...
    LIST_FOR_EACH_POP (a, list_node, act_list) {
        hmap_remove(a->flow->uuid_action_flow_table,
                    &a->uuid_action_node);
        ovn_flow_action_destroy(a);
    }
}
...
{
    if (f->normal_act) {
        hmap_remove(f->uuid_action_flow_table,
                    &f->normal_act->uuid_action_node);
...
{
    hmap_remove(f->uuid_action_flow_table, &a->uuid_action_node);
...
{
    hmap_insert(&ftable->match_flow_table, &f->hmap_node, f->hash);
                        ...
                        struct ovn_flow *f, struct ovn_flow_action *a)
{
    ...
    hmap_insert(&ftable->uuid_action_flow_table, &a->uuid_action_node,
...
static bool
ovn_flow_table_is_empty(struct ovn_flow_table *ftable)
{
    return hmap_is_empty(&ftable->match_flow_table);
}
                               ...
                               struct ovs_list *msgs)
{
    ...
    HMAP_FOR_EACH_POP (f, hmap_node,
                       &dp_ftable->old_lflow_table->match_flow_table) {
        ovn_flow_clear_actions(f);
    ...
    HMAP_FOR_EACH_POP (f, hmap_node,
                       &dp_ftable->old_pflow_table->match_flow_table) {
        ovn_flow_clear_actions(f);
        ovn_flow_prepare_ofmsg(f, msgs);
...
dp_flow_print_oflows__(struct dp_flow_table *dp_ftable, FILE *stream)
{
    ...
    if (hmap_is_empty(&dp_ftable->active_lflow_table->match_flow_table)) {
        fputs("empty\n", stream);
    } else {
        fputc('\n', stream);
        HMAP_FOR_EACH (f, hmap_node,
                       &dp_ftable->active_lflow_table->match_flow_table) {
            char *s = ovn_flow_to_string(f);
            if (s) {
                fputs(s, stream);
                free(s);
    ...
    if (hmap_is_empty(&dp_ftable->old_lflow_table->match_flow_table)) {
        fputs("empty\n", stream);
    } else {
        fputc('\n', stream);
        HMAP_FOR_EACH (f, hmap_node,
    ...
    if (hmap_is_empty(&dp_ftable->active_pflow_table->match_flow_table)) {
        fputs("empty\n", stream);
    } else {
        HMAP_FOR_EACH (f, hmap_node,
                       &dp_ftable->active_pflow_table->match_flow_table) {
            char *s = ovn_flow_to_string(f);
            fputs(s, stream);
            free(s);
            fputc('\n', stream);
    ...
    if (hmap_is_empty(&dp_ftable->old_pflow_table->match_flow_table)) {
        fputs("empty\n", stream);
    } else {
        fputc('\n', stream);
        HMAP_FOR_EACH (f, hmap_node,
