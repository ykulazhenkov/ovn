...
struct ovn_flow {
    ...
    struct ovs_list flow_list_node;

    /* Flow list in which this 'ovn_flow' is present. */
...
struct dp_flow_table {
    ...
    struct ovs_list *active_lflows;
    struct ovs_list *old_lflows;
...
void
dp_flow_print_oflows(uint32_t dp_key OVS_UNUSED, FILE *stream OVS_UNUSED)
{
#if 0
    struct dp_flow_table *dp_ftable = dp_flow_table_get(dp_key);
    if (dp_ftable) {
                       ...
                       const struct uuid *flow_uuid)
{
    ...
    if (!f) {
        ...
        f->match_flow_table = &ftable->match_flow_table[table_id];
                ...
                const struct minimatch *match)
{
    ...
    HMAP_FOR_EACH_WITH_HASH (f, hmap_node, hash,
                             &flow_table->match_flow_table[table_id]) {
