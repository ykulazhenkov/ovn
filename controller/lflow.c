...
add_lb_vip_hairpin_flows(struct ovn_controller_lb *lb,
                         struct ovn_controller_lb_vip *lb_vip,
                         struct ovn_controller_lb_vip_backend *lb_backend,
                         uint8_t lb_proto,
                         struct ovn_desired_flow_table *flow_table)
{
    uint64_t stub[1024 / 8];
                          ...
                          struct ovn_desired_flow_table *flow_table)
{
    ...
    for (i = 0; i < lb->n_vips; i++) {
        struct ovn_controller_lb_vip *lb_vip = &lb->vips[i];

        for (size_t j = 0; j < lb_vip->n_backends; j++) {
            struct ovn_controller_lb_vip_backend *lb_backend;

            lb_backend = &lb_vip->backends[j];
