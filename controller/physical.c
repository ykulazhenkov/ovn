...
physical_run(struct physical_ctx *p_ctx)
{
    ...
    ofctrl_add_flow(p_ctx->flow_table, OFTABLE_SAVE_INPORT, 0, 0, &match, &ofpacts,
                    hc_uuid);
...
void
physical_handle_ovs_interface_changes(struct physical_ctx *p_ctx OVS_UNUSED)
{
    VLOG_INFO("NUMS : %s : %s : %d entered", __FILE__, __FUNCTION__, __LINE__);
    bool run_physical_run = false;
    const struct ovsrec_interface *iface_rec;
    OVSREC_INTERFACE_TABLE_FOR_EACH_TRACKED (iface_rec, p_ctx->iface_table) {
        if (!strcmp(iface_rec->type, "geneve") ||
            !strcmp(iface_rec->type, "patch")) {
            run_physical_run = true;
            break;
        }
    }

    if (run_physical_run) {
        VLOG_INFO("NUMS : %s : %s : %d : phsyical run required.", __FILE__, __FUNCTION__, __LINE__);
        physical_run(p_ctx);
    ...
    OVSREC_INTERFACE_TABLE_FOR_EACH_TRACKED (iface_rec, p_ctx->iface_table) {
        VLOG_INFO("NUMS : %s : %s : %d : iface = [%s]", __FILE__, __FUNCTION__, __LINE__, iface_rec->name);
        ...
        const struct sbrec_port_binding *pb =
            lport_lookup_by_name(p_ctx->sbrec_port_binding_by_name, iface_id);

        if (!pb) {
            continue;
        }

        VLOG_INFO("NUMS : %s : %s : %d found pb [%s] for iface [%s]",
                  __FILE__, __FUNCTION__, __LINE__, pb->logical_port, iface_rec->name);

        if (ovsrec_interface_is_deleted(iface_rec)) {
            ofctrl_remove_flows(p_ctx->flow_table, &pb->header_.uuid);
        } else {
            if (!ovsrec_interface_is_new(iface_rec)) {
                ofctrl_remove_flows(p_ctx->flow_table, &pb->header_.uuid);
            }
            VLOG_INFO("NUMS : %s : %s : %d : calling consider_port_binding", __FILE__, __FUNCTION__, __LINE__);
            consider_port_binding(p_ctx->sbrec_port_binding_by_name,
                                  p_ctx->mff_ovn_geneve, p_ctx->ct_zones,
                                  p_ctx->active_tunnels, p_ctx->local_datapaths,
                                  pb, p_ctx->chassis,
